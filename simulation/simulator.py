...
class Simulator:
    ...
    def __init__(self, address='../dataset/train/calgary', w=True):
        self.step = 0
        self.end = 360000000
        self.vehicles = 5
        sumoBinary = "sumo-gui"
    ...
    def write(self):
        for i in range(0, self.vehicles):
            ...
            plt.plot(range(0, len(self.x[i])), self.s[i], label=f'Vehicle #{i + 1}')
            self.file.write(l)
            self.file.write(';')
    ...
    def log(self):
        ...
        for i in range(0, self.vehicles):
            for id in traci.vehicle.getIDList():
                if re.search(f'^calgary[0-3]{self.round * 10 + i}', id):
                    ...
                    if 'calgary0' in id:
                        ...
                        self.x[i].append(self.normalize(x, 3065.342275320151, 2772.2621071169474))  # edmonton
                        self.y[i].append(self.normalize(y, 2131.654558026798, 1363.8018146710729))
                    elif 'calgary2' in id:
                        self.x[i].append(1 - self.normalize(x, 2152.7796663612908, 545.5903406507975))  # 9ave
                        self.y[i].append(self.normalize(y, 464.0185642886375, 449.11312340811094))
                    elif 'calgary3' in id:
                        self.x[i].append(self.normalize(x, 1313.5504353390693, 824.1861067955539))  # sunny
                        self.y[i].append(self.normalize(y, 1546.6093807856416, 1313.0086373723486))
